...
用户专用的 Git 提交流程技能。封装了 Git 提交流程和约定的提交规范。
...
- **`/commit <repo-path>`**（如 `/commit ai-playbook`）→ 路径指向 Git 仓库根目录，提交该仓库
...
部分文件可能被多个会话/Agent 同时修改（如 `config.js` 导航配置、公共常量、README 等）。当此类文件的 `git diff` 中包含**非本次会话的改动**（其他 Agent 或人员在同一文件中的修改，你并没有改过这些行）时：

1. **备份**：`cp <file> <file>.backup`（保留含他人改动的完整版本）
2. **还原**：`git checkout -- <file>`（回到 HEAD 状态）
3. **重写**：仅写入本次会话对该文件的修改内容
4. **暂存并提交**：正常走后续流程
5. **恢复**：`mv <file>.backup <file>`（他人改动回到工作区，未暂存，不影响下次提交）

在步骤 6（用户确认）展示提交信息时，需一并展示：

```
⚠️ 以下文件含有非本次会话的改动，已仅提交本次修改，
   他人改动保留在工作区（未暂存）：
   - <file>：<简述他人改动内容>
```
...
- 动词+内容格式，简洁明了
- 不包含引号
- **只描述仓库变更内容，不提数据来源**（如"从 XX 迁移"、"合并自 XX"等）。提交信息只反映 diff 中的实际变更，数据从哪来的不重要
...
检查未暂存修改       ← 对每个 modified 文件 git diff <file>
  ↓
检查含他人改动的文件  ← 如有，备份→还原→仅写本次修改→提交后恢复
  ↓
...
编写提交信息         ← 基于实际变更，纯中文，动词+内容
  ↓
展示给用户确认       ← 含他人改动提醒，必须等待用户批准
  ↓
git commit -F $TMP/commit_msg.txt   ← 使用 UTF-8 文件，禁止中文消息走 echo/printf 管道
  ↓
恢复他人改动         ← mv <file>.backup <file>（如有）
  ↓
...
本技能约束源自 `general-conventions.md` 中的 Git 提交规范。
